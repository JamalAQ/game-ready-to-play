...
// for the helth of the player 
let playerHealthDiv= document.querySelector(`#playerHealth`)
...
setInterval(()=>{
  ...
  function move_up() {
    ...
    console.log(theplayer_and_theWapon.style.top.slice(0,-2))
  function move_down() {
    // if(detecterAutoRepeatKey === false ) {
...
let generateHelthCure = 0
let helthCures = document.querySelector(`#helthCure`)

function generatehelthCure() { 
  let helthCure = document.createElement(`div`)
  helthCures.appendChild(helthCure)
  helthCure.classList.add(`helthCure`)
  helthCure.style.top = `${ Math.floor((Math.random() * 4500) + 100)}px`
  helthCure.style.left = `${Math.floor((Math.random() * 3700) + 100)}px`
  generateGoldTims += 1
}

setInterval(generatehelthCure, 3000)
...
// setInterval (autoincresHelth , 1000)


// #########################################  incresed by colect 


let helth_in_the_map = []

setInterval(()=> {
  helth_in_the_map = document.querySelectorAll(`.helthCure`)
} , 1000)


// ######################################## colect helth

setInterval(()=>{if(helthColectAbility===false){helthColectAbility=true}},1000)
  helthColectAbility=true;
setInterval(
  ()=>{helth_in_the_map.forEach((e)=>{
  
  // let player_top = +the_player.style.top.slice(0,+the_player.style.top.length-2)

  let helth_item_top=+e.style.top.slice(0, +e.style.top.length-2)

  // let player_left = +the_player.style.left.slice(0,+the_player.style.left.length-2 )

  let helth_item_left = +e.style.left.slice(0, +e.style.left.length-2)


  if (   a >= helth_item_top-25 && a <= helth_item_top+25   ) 
  
  {
    if(d >= helth_item_left-25 && d <= helth_item_left+25 ){


      if(helthColectAbility){
        if(playerHealth <= 90){
          e.remove() ; 
          playerHealth += 10;
          playerHealthDiv.style.width= `${playerHealth}%`
...
//return to the pont of monster it self
